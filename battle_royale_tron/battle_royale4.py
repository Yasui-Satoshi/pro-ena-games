...
class Player:                       #プレイヤー設定
    ...
    def __draw__(self):             # 軌跡設定
        self.rect = pg.Rect(self.x,self.y,P,P)  # プレイヤーオブジェクト(2*2)
        pg.draw.rect(screen, self.colour, self.rect, 1)        # プレイヤーのマスを塗る
    
    def __move__(self):             # 移動設定
        print('move')
        print(self.bearing[0],self.bearing[1])
...
all_cell  = list() # セル情報(プレイヤー番号あり)
wall_cell = list() # セル情報(プレイヤー番号なし)
